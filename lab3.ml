...
let transcript (enrollments : enrollment list)
               (student : int)
             : enrollment list =
  List.filter (fun enrollment -> enrollment.id = student) enrollments ;;
...
let ids (enrollments: enrollment list) : int list =
    List.map (fun e -> e.id) (List.sort_uniq (fun e1 e2 -> e1.id - e2.id) enrollments) ;;
...
let verify (enrollments : enrollment list) : bool =
  List.for_all (fun x -> List.for_all (fun y -> if (x.id = y.id) then x.name = y.name else true) enrollments) enrollments ;;
...
let rec partition (f : 'a -> bool) (lst : 'a list) : 'a list * 'a list =
   ...
   | [] -> [], []
   | hd :: tl -> 
    match partition f tl with
    | [], [] -> if f hd then ([hd], []) else ([], [hd])
    | (tru, fal) -> 
      if f hd then (hd :: tru, fal) else (tru, hd :: fal) ;;
